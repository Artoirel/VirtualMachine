        ...
        case RV64_OP_OP_IMM:
            switch(instruction.i_type.funct3) {
                ...
                case RV64_FUNCT3_SLTIU:
                    unsigned long sltiuval = read_reg_long(instruction.i_type.rs1);
                    unsigned long sltiuimm = instruction.i_type.imm;
                    uint64_t writeval = 0;
        ...
        case RV64_OP_OP_IMM32:
            switch(instruction.i_type.funct3) {
                ...
                case RV64_FUNCT3_SRLIW:
                    uint32_t srliw_val = read_reg_int(instruction.i_type.rs1) >> i_imm(instruction.i_type);
                    uint64_t srliw_write = srliw_val;
                    if (srliw_val >> 31 == 1)
                    {
                        srliw_write = 0xFFFFFFFF00000000l | srliw_val;
                    }

                    write_reg_long(instruction.i_type.rd, srliw_write);
                    return PC + 4;// 0x0
            }
            printf("\n");
        ...
        case RV64_OP_SYSTEM:
            ...
            switch (sys)
            {
                ...
                case RV64_SYS_readlinkat:
                    ...
                    write_arbitrary_bytes(fullpath, read_reg_long(12), strlen(fullpath));
                    write_reg_long(10, strlen(fullpath));
